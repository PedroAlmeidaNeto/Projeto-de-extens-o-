...
export const VirtualAssistant: React.FC<VirtualAssistantProps> = ({ isOpen, onClose, clients, pets, appointments }) => {
    ...
    useEffect(() => {
        if(isOpen && messages.length === 0) {
            setMessages([{ role: 'model', content: "Olá! Eu sou a Patty, sua assistente virtual da Pat Pet Shop. Como posso ajudar hoje?" }]);
        }
    }, [isOpen, messages]);

    const systemInstruction = `
        Você é a Patty, uma assistente virtual amigável e prestativa do Pat Pet Shop. Seu objetivo é fornecer um excelente atendimento ao cliente.
        - Seja concisa, educada e profissional.
        - Use os dados fornecidos do pet shop para responder às perguntas com precisão.
        - Se você não souber a resposta ou se uma solicitação for muito complexa (como diagnósticos médicos), peça educadamente ao usuário para entrar em contato com o pet shop diretamente pelo telefone (15) 99999-8888.
        ...
        Aqui estão os dados atuais do pet shop em formato JSON:
    ...
    return (
        <div className="fixed bottom-24 right-8 w-96 h-[60vh] bg-white rounded-2xl shadow-2xl flex flex-col z-40 transition-all duration-300 ease-in-out">
            <header className="bg-slate-500 text-white p-4 rounded-t-2xl flex justify-between items-center">
                <div className="flex items-center">
                    <PawIcon className="w-7 h-7 mr-2"/>
                    <h3 className="text-lg font-bold">Assistente Virtual Patty</h3>
                </div>
                 <button onClick={onClose} className="text-white hover:text-slate-100">&times;</button>
            </header>
            <main className="flex-1 p-4 overflow-y-auto bg-slate-50">
                <div className="space-y-4">
                    {messages.map((msg, index) => (
                        <div key={index} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] p-3 rounded-lg ${msg.role === 'user' ? 'bg-slate-500 text-white rounded-br-none' : 'bg-gray-200 text-gray-800 rounded-bl-none'}`}>
            ...
            <footer className="p-3 border-t bg-white rounded-b-2xl">
                <div className="flex">
                    <input
                        ...
                        className="flex-1 border border-gray-300 rounded-l-md p-2 focus:ring-slate-500 focus:border-slate-500"
                        disabled={isLoading}
                    />
                    <button onClick={handleSend} className="bg-slate-500 text-white px-4 rounded-r-md hover:bg-slate-600 disabled:bg-slate-300" disabled={isLoading || !input.trim()}>
                        Enviar
                    </button>
                </div>
            </footer>
